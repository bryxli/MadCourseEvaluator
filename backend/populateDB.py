...
def PopProfessorsHelper(professor_data, professor_name_list_RMP):
    """
    Helper function to get the professor from RMP and populate the professors table with all professors at UW-Madison.
...
def PopProfessorsHelper_2(professor_data):
    """
    Helper function to get the professor from MadGrades and populates professors table with all professors at UW-Madison.
...
def PopProfessorsHelper_3(professor_name_list_RMP):
    """
    Helper function to get the professor from MadGrades and populates professors table with all professors at UW-Madison.
    ...
    file = open('./compsci_test_sample/test_course.json', 'r') # Open the JSON file containing all UW-Madison courses (pre-scraped)
    data = json.load(file)                  # Load the JSON file into a dictionary
...
def PopProfessors():
    ...
    professors_data.append(api_2.ScrapeProfessors())
    for data in professors_data:
        PopProfessorsHelper(data, professor_name_list_RMP)

    PopProfessorsHelper_3(professor_name_list_RMP)
